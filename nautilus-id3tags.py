...
class Tag:
    ...
    def __init__(self, tag_name, read_method, save_method, initial_value):
        self.tag_name = tag_name
        self.read_method = read_method
        self.save_method = save_method
        self.save_method(initial_value)

    def save(self):
        save_method(read_method)
...
class NautilusID3Tags(GObject.GObject, Nautilus.PropertyPageProvider):
    ...
    def get_property_pages(self, files):
        ...
        try:
            self.audio_file = taglib.File(filename)
        except:
            return
        ...
        for idx, genre in enumerate(self.__class__.genres):
            self.genre_combo.append(str(idx), genre)
    ...
    def save_tag(self, tag_name, tag_value):
        self.song_tags[tag_name] = tag_value
    

    def load_data(self):
        print(self.audio_file.tags)
        self.length_value.set_text(self.convert_s_to_human(self.audio_file.length))
        self.tags.append(Tag("title", self.title_entry.get_text, self.title_entry.set_text, self.audio_file.tags["TITLE"][0].decode()))
        self.tags.append(Tag("artist", self.artist_entry.get_text, self.artist_entry.set_text, self.audio_file.tags["ARTIST"][0].decode()))
        self.tags.append(Tag("album", self.album_entry.get_text, self.album_entry.set_text, self.audio_file.tags["ALBUM"][0].decode()))
        self.tags.append(Tag("genre", self.genre_combo.get_active_text, self.genre_combo.set_active, self.__class__.genres.index(self.audio_file.tags["GENRE"][0])))
        self.tags.append(Tag("date", self.date_entry.get_text, self.date_entry.set_text, self.audio_file.tags["DATE"][0].decode()))

    def convert_s_to_human(self, seconds):
        x = seconds / 60
        seconds = seconds % 60
